...
Migrator.prototype.create = function(basename, callback) {
    ...
    self._getNextNumber(function(err, num) {
        ...
        fs.readFile(self.params.template, function(err, content) {
            ...
            fs.writeFile(self.getMigrationPathByName(name), content, function(
                err
            ) {
                callback(err, name)
            })
...
Migrator.prototype._executeAction = function(action, callback) {
    if (this.isAsyncAction(action)) {
        ...
        callback = function(err) {
            err =
                err ||
                (callbackRunned && new Error('Callback called more than once'))
            if (err) {
                oldCallback(err)
                return
            }
...
Migrator.prototype._executeMigration = function(migration, action, callback) {
    var self = this
    self._executeAction(migration[action], function(err) {
        if (err) {
            err.message =
                'Error during ' +
                action +
        ...
        if (migration.force) {
            self.adapter.getExecutedMigrationNames(function(
                err,
                executedNames
            ) {
                if (err) {
            ...
            self.adapter[
                action == 'migrate' ? 'markExecuted' : 'unmarkExecuted'
                ...
                if (number) {
                    if (number in numbersHash) {
                        if (numbersHash[name].length == 1) {
                            ...
                            throw new Error(
                                'Specified migration name `' +
                                    name +
                                    '` is ambiguous: ' +
                                    numbersHash[number].join(', ') +
                                    ' please choose one'
                            )
                        }
                    }
